    ...
    if [ -z $KPKUBECONFIG ]
        then
            ...
            then
                ...
                if [ $KPKUBECONFIG_SUBJECT == "Subject:CN=system:kube-proxy" ] && [ $KPKUBECONFIG_ISSUER == "Issuer:CN=KUBERNETES-CA" ] && [ $KPKUBECONFIG_CERT_MD5 == $KPKUBECONFIG_KEY_MD5 ] && [ $KPKUBECONFIG_SERVER == "https://192.168.0.139:6443" ]
                    then
    ...
    if [ -z $ETCDCERT ] && [ -z $ETCDKEY ]
        then
            ...
            then
                ...
                INTERNAL_IP=$(ip addr show eno1 | grep "inet " | awk '{print $2}' | cut -d / -f 1)
        ...
        then
            ...
            then
                ...
                if [ $WORKER_1_KUBECONFIG_SUBJECT == "Subject:CN=system:node:worker-1,O=system:nodes" ] && [ $WORKER_1_KUBECONFIG_ISSUER == "Issuer:CN=KUBERNETES-CA" ] && \
                   [ $WORKER_1_KUBECONFIG_CERT_MD5 == $WORKER_1_KUBECONFIG_KEY_MD5 ] && [ $WORKER_1_KUBECONFIG_SERVER == "https://192.168.0.139:6443" ]
                    then
